<template>
  ...
  item-key="name" 
    ...
    :footer-props="{
      ...
      itemsPerPageText:'Total Users Pages:',
            ...
            >
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                        <v-col cols="12" sm="12">
                          <v-text-field :rules="[rules.required,rules.min]" v-model="editedItem.name" label="User Name"></v-text-field>
                        </v-col>
                    </v-row>
                    
                    <v-row v-if="editedIndex == -1">
                        <v-col cols="12" sm="12">
                          <v-text-field :rules="[rules.required]" v-model="editedItem.password" label="Type Password" type="password"></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="12">
                          <v-text-field :rules="[rules.required,passMatch]" v-model="editedItem.rpassword" label="Retype Password" type="password"></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="12">
                          <v-text-field type="email" :success-messages="success" :error-messages="error" :blur="emailCheck" :rules="[rules.required,rules.vEmail]" v-model="editedItem.email" label="Email"></v-text-field>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="12" sm="12">
                          <v-select v-model="editedItem.role" :items="roles" :rules="[rules.required]" color="error" label="Select Role"></v-select>
                        </v-col>
                    </v-row>
    ...
    <template v-slot:item.role = "{ item }">
        ...
        >
            ...
            <template v-slot:input>
                  <v-select v-model="item.role" :items="roles" :rules="[rules.required]" color="error" label="Select Role"></v-select>
            </template>
        </v-edit-dialog>
    </template>
...
<script>
  export default {
    data: () => ({
      ...
      rules:{
        required: v => !!v || 'You have to fill up this fields.',
        min: v=> v.length >=5 || 'Please enter valid name.',
        vEmail: v=> /.+@.+\..+/.test(v) || 'Invalid Email !!!'
      },
      headers: [
    ...
    methods: {
      ...
      updateRole(item)
      {
          ...
          .catch(err=>{
              this.text =err.response.data.user.name + "'s role can not be changed "
              this.color= 'error'
      ...
      searchIt(e){
        if(e.length > 2){
          axios.get(`/api/users/${e}`)
              .then(res => this.users.data = res.data.users)
